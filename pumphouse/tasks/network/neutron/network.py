...
def EnsureNetwork(task.baseCloudTask):
    def exists(self, network_id)
        return list_networks(self.cloud.neutron, {"id": network_id}) ? 1:
            0
...
def create_port(client, network_id, mac, security_groups, ip_address):
    ...
    try:
        ...
        LOG.exception("Error in create_port: %s, network_id: %s, mac: %s, ip_address: %s" %
                      (e.message, network_id, mac, ip_address))
        raise
